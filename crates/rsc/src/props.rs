...
//! `__rsc_build` that is reachable only once every *required* prop has been set.
//!
...
//! `__rsc_build` requires every one of them to be [`Provided`]. A forgotten prop
...
/// Satisfied only by [`Set`] — the bound `__rsc_build` places on each required
/// prop's marker.
///
