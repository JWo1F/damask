...
/// 2. The sibling convention — `<name_snake>.*.dmk` in the source file's own
///    directory.
    ...
    match matches.len() {
        ...
        0 => Err(format!(
            "no template found for component `{name_snake}`: expected a file named \
             `{name_snake}.dmk` next to the struct{}. \
...
fn manifest_dir() -> Result<PathBuf, String> {
    std::env::var_os("CARGO_MANIFEST_DIR")
        .map(PathBuf::from)
        .ok_or_else(|| {
            "CARGO_MANIFEST_DIR is not set, so Damask cannot locate templates. \
...
/// `.dmk` files directly in `dir` whose component-basename equals `name_snake`.
...
/// The component-basename of a template file: its name without the `.dmk`
/// extension (`greeting.dmk` → `greeting`). Returns `None` for non-`.dmk` files.
pub fn component_basename(file: &str) -> Option<String> {
    file.strip_suffix(".dmk").map(str::to_string)
}

/// Recursively list every `.dmk` file under `root`, memoized per crate compile.
...
mod tests {
    ...
    #[test]
    fn basename_strips_dmk() {
        assert_eq!(
            component_basename("greeting.dmk").as_deref(),
            Some("greeting")
        );
        assert_eq!(
            component_basename("my_button.dmk").as_deref(),
            Some("my_button")
        );
    ...
    fn unique_dir() -> PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("damask_resolve_{}_{n}", std::process::id()));
    ...
    fn resolves_sibling_template_by_name() {
        ...
        std::fs::write(dir.join("widget.dmk"), "hi {self.x}").unwrap();
    ...
    fn explicit_template_path_wins() {
        ...
        let rs = dir.join("widget.rs");
        std::fs::write(&rs, "// component").unwrap();
        std::fs::write(dir.join("custom.dmk"), "<b>{self.x}</b>").unwrap();

        let resolved = resolve(Some(&rs), "widget", Some("custom.dmk")).expect("resolves");
