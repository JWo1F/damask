...
//!
//! A template lowers `<Card title={t}/>` to `Card::__rsc_props().title(t)
//! .__rsc_build()` rather than a struct literal, because the call site knows
...
//! a quoted value needs happens on the value side instead — `rsc::props` has the
...
//! that starts as a marker named after it and flips to `rsc::props::Set` when
//! its setter runs; `__rsc_build` demands `Provided` of each, so the diagnostic
...
pub fn expand(input: &DeriveInput, defaulted: bool) -> TokenStream {
        ...
        .map(|(i, field)| {
            ...
            Prop {
                ...
                tracked: (!defaulted && !is_skippable(&field.ty)).then(|| {
                    (
                        format_ident!("__RscM{i}"),
                        format_ident!("__Rsc{name}_{ident}"),
                    )
                }),
            }
        })
    ...
    let builder = format_ident!("__RscProps{name}");
    let store = format_ident!("__RscPropStore{name}");
    ...
    let setters = props.iter().map(|prop| {
        ...
        quote! {
            #[doc(hidden)]
            #field_vis fn #ident(mut self, __rsc_value: #ty) -> #returns {
                self.__rsc_store.#ident = ::core::option::Option::Some(__rsc_value);
                #builder {
                    __rsc_store: self.__rsc_store,
                    __rsc_state: ::core::marker::PhantomData,
                }
            }
        }
    });

    // The bounds sit on `__rsc_build` itself rather than on its impl block, so
    ...
    let (build_where, build_body) = if defaulted {
        ...
        (
            ...
            quote! {
                let __rsc_store = self.__rsc_store;
                let mut __rsc_out = <#name #comp_ty as ::core::default::Default>::default();
                #(
                    if let ::core::option::Option::Some(__rsc_value) = __rsc_store.#names {
                        __rsc_out.#names = __rsc_value;
                    }
                )*
                __rsc_out
            },
        ...
        let values = props.iter().map(|prop| {
            ...
            match prop.tracked {
                // Unreachable: the `Provided` bounds on `__rsc_build` are
                // exactly the proof that every required prop's setter has run.
                Some(_) => quote! {
                    #ident: match __rsc_store.#ident {
                        ::core::option::Option::Some(__rsc_value) => __rsc_value,
                        ::core::option::Option::None => ::core::unreachable!(),
                    }
                },
                // A skippable prop is an `Option<_>`, whose `Default` is `None`
                // whatever it wraps.
                None => quote!(#ident: __rsc_store.#ident.unwrap_or_default()),
            }
        });
        let bounds = if params.is_empty() {
            ...
            quote!(where #(#params: ::rsc::props::Provided,)*)
        };
        (
            bounds,
            quote! {
    ...
    quote! {
        ...
        #vis struct #store #store_decl #comp_where {
            ...
            __rsc_component: ::core::marker::PhantomData<fn() -> #name #comp_ty>,
        }

        #[doc(hidden)]
        #[allow(non_camel_case_types)]
        #vis struct #builder #builder_decl #comp_where {
            __rsc_store: #store #store_args,
            __rsc_state: ::core::marker::PhantomData<fn() -> (#(#held,)*)>,
        }
        ...
        impl #comp_impl #name #comp_ty #comp_where {
            #[doc(hidden)]
            #vis fn __rsc_props() -> #unset_builder {
                #builder {
                    __rsc_store: #store {
                        #( #names: ::core::option::Option::None, )*
                        __rsc_component: ::core::marker::PhantomData,
                    },
                    __rsc_state: ::core::marker::PhantomData,
                }
            }
        ...
        impl #setter_impl #held_builder #comp_where {
            ...
            #[doc(hidden)]
            #vis fn __rsc_build(self) -> #name #comp_ty #build_where {
                #build_body
            }
        }
    }
