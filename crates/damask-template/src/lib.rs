//! Template parser for **Damask**.
//!
//! Damask templates are HTML with a `{ … }` tag syntax, JSX-style
//! `<Component/>` elements, and `<slot/>`s. This crate parses a `.dmk` template
...
/// exact byte range in the `.dmk` source.
///
...
pub enum AttrValue {
    ...
    /// The expression yields anything implementing `damask::AttrSpread`, which is
