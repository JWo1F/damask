//! The HTML skeleton projected from a `.dmk` template for an HTML language
//! server.
//!
...
//! map back to the `.dmk` by the identity — no source map needed.
//!
...
use damask_template::tag_spans;

/// Project `damask_src` to an HTML skeleton by blanking every `{ … }` tag. The
/// result has the same length and line structure as the input.
pub fn html_skeleton(damask_src: &str) -> String {
    let mut bytes = damask_src.as_bytes().to_vec();
    for span in tag_spans(damask_src) {
