...
//!
//! `damask-lsp` is itself a server (to the editor), but to give template code real
...
mod tests {
    ...
    /// PATH. Run with: `cargo test -p damask-lsp -- --ignored ra_hover`.
    #[tokio::test]
    #[ignore = "spawns rust-analyzer; slow, needs network-free crate index"]
    async fn ra_hover_resolves_self_field_through_overlay() {
        // A minimal crate that depends on `damask`, laid out in a temp dir.
        let dir = std::env::temp_dir().join(format!("damask-lsp-ra-test-{}", std::process::id()));
        let src = dir.join("src");
        std::fs::create_dir_all(&src).unwrap();
        let damask_crate = concat!(env!("CARGO_MANIFEST_DIR"), "/../../crates/damask");
        std::fs::write(
            dir.join("Cargo.toml"),
            format!(
                "[package]\nname = \"ra_probe\"\nversion = \"0.0.0\"\nedition = \"2021\"\n\
                 [dependencies]\ndamask = {{ path = {damask_crate:?} }}\n",
            ),
        )
        .unwrap();
        let rs = "use damask::Component;\n\n#[derive(Component)]\npub struct Greeting {\n    pub name: String,\n}\n";
        std::fs::write(src.join("lib.rs"), rs).unwrap();

        let template = damask_template::parse("Hello {self.name}!").unwrap();
        ...
        // Position of `name` in the .dmk, mapped to the overlay, then to LSP
    ...
    async fn ra_completion_offers_self_members_through_overlay() {
        let dir = std::env::temp_dir().join(format!("damask-lsp-ra-compl-{}", std::process::id()));
        let src = dir.join("src");
        std::fs::create_dir_all(&src).unwrap();
        let damask_crate = concat!(env!("CARGO_MANIFEST_DIR"), "/../../crates/damask");
        std::fs::write(
