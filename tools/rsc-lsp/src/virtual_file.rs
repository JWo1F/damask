...
//! `.rsc` template is not one. The trick is to never show it a `.rsc` at all:
...
//! the `.rsc` source to the appended region, so a request at a template position
//! can be forwarded to the equivalent overlay position and results mapped back.

use rsc_template::{Span, Template, lower_mapped};
...
const CHECK_FN: &str = "__rsc_check";

/// A verbatim, equal-length correspondence between a `.rsc` source range and a
...
impl VirtualFile {
    ...
    pub fn build(rs_src: &str, struct_name: &str, template: &Template) -> Result<Self, String> {
        ...
        // raise (its `self`/`__rsc` uses, redundant braces, and so on).
        text.push_str(
    ...
    /// Translate a byte offset in the `.rsc` source to the overlay, if it lands
...
mod tests {
    ...
    fn build(rsc: &str) -> (VirtualFile, String) {
        let template = rsc_template::parse(rsc).unwrap();
